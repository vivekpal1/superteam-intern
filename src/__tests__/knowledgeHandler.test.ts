...
export class KnowledgeHandler {
    ...
    async handleDocument(ctx: Context): Promise<void> {
        ...
        try {
            ...
            if (doc.file_size && doc.file_size > this.maxFileSize) {
            ...
            const content = await this.processDocument(file.file_path!);
    ...
    async handleQuery(ctx: Context, query: string): Promise<void> {
        ...
        try {
            ...
            metrics = {
                confidence,
                sources: relevantDocs.map((doc: RelevantDoc) => doc.metadata.source),
            ...
            await this.saveInteraction(ctx.message as Message, response, metrics);
        } catch (error) {
    ...
    private calculateConfidence(docs: RelevantDoc[]): number {
    ...
    private prepareContext(docs: RelevantDoc[]): string {
        return docs
            .map(doc => this.sanitizeContent(doc.content))
            .join('\n\n')
            .slice(0, 3000);
    }
...
Answer:`;
    ...
    private async saveInteraction(message: Message, response: string, metrics: QueryMetrics | null) {
        if (!message.from) {
            return; // Skip saving if there's no user information
        }
        ...
        await this.prisma.conversation.create({
            data: {
                ...
                messages: [
                    { 
                        role: 'user', 
                        content: 'text' in message ? message.text : 'Non-text message' 
                    },
                    { 
                        role: 'assistant', 
                        content: response 
